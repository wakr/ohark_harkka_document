...
Interpreted, no threads, 
\paragraph{Sovelluksen vaikea deployaus} 
Ruby on Rails vaatii paljon esiasennusta + itsessään isohko järjestelmä. Ruby ei ole tuettuina koneissa automaattisesti. Rails pitää itsessään asentaa

\paragraph{Legacy-ongelmat} 
Konventiot.
\paragraph{Rajoittuneisuus}
Konventiot.

\paragraph{Sopiiko suunniteltu arkkitehtuuri järjestelmälle?}
\paragraph{Mikä vaihtoehtoisista arkkitehtuureista sopii parhaiten järjestelmälle
ja miksi?}
\paragraph{Miten hyvä tulee olemaan järjestelmän jokin tietty laadullinen
ominaisuus, olettaen järkevä toteutustapa?}
